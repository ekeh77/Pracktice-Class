...
<script>
	...
	function showList(){
		...
		function setList(datas){
			...
			strHTML += "<span class='gName'>게임명</span>"
			strHTML += "<span class='gPrice'>가격</span>"
			strHTML += "</div>"
	...
	function chk_buy(strID){
		if(strID == 'null'){
			...
			var gName = document.getElementsByName("gName");
			var gPrice = document.getElementsByName("gPrice");
			var gNameInput = document.getElementById("buyGameName");
			var gPriceInput = document.getElementById("buyGamePrice");
			/*
			var arrGName = new Array();
			var arrGPrice = new Array();
			*/
			
			for(var i = 0; i < chk.length; i++){
				if(chk[i].checked){
					gNameInput.value = gName[i].innerHTML; 
					gPriceInput.value = gPrice[i].innerHTML.substr(0,gPrice[i].innerHTML.length - 1);
					countBuy += 1;
				}
				else{
...
<body onload="showList(); chkLogin('<%=strResult %>');">
	<div id="main">
		...
		<div id="mid">
				<div id="showList"></div>
				<div id="buyBtn">
					<input type="button" value="구매" onclick="chk_buy('<%=strUserID%>');"/>
					<form action="./buyForm.jsp" method="post" id="buyForm">
						<input type="hidden" name="buyGameName" value="" id="buyGameName"/>
						<input type="hidden" name="buyGamePrice" value="" id="buyGamePrice"/>
					</form>
				</div>
		</div>
	</div>
