...
<script>
	...
	var showGameList = function(){
		...
		$.ajax({
			...
			success:function(data){
                 ...
                 strHTML += '<form action="../GS_InsertBag" method="post" id="buyGame">'
                 for(var i=0;i<gameLists.length;i++){
                    ...
                    strHTML += '<span>';
                    strHTML += '<input type="checkbox" name="GLchk" class="GListInput" id="chk'+ i +'" value="' + G_ID + '">';
                    strHTML += '</span>';
                    strHTML += '<span ><input type="text" name="GName" onclick="chkListOn('+ i +');" class="GListInput" value="' + G_NAME + '" readonly/></span>';
                    strHTML += '<span><input type="text" name="GMaker" onclick="chkListOn('+ i +');" class="GListInput" value="' + G_MAKER + '" readonly/></span>';
                    strHTML += '<span><input type="text" name="GRelease" onclick="chkListOn('+ i +');" class="GListInput" value="' + G_RELEASE + '" readonly/></span>';
                    strHTML += '<span><input type="text" name="GSalesPrice" onclick="chkListOn('+ i +');" class="GListInput" value="' + G_SALES_PRICE + '" readonly/></span>';
                    strHTML += '<span><input type="text" name="GGenre" onclick="chkListOn('+ i +');" class="GListInput" value="' + G_GENRE + '" " readonly/></span>';
                    strHTML += '</div>';
                    
                 }
                 strHTML += '<input type="hidden" name="ctID" value="' + userID + '">';
                 strHTML += '</form>'
                 strHTML += '</div>';
                 strHTML += '<div id="Buybutton">';
                 strHTML += '<input type="button" value="장바구니로가기" onclick="chk_buy(&quot;' + userID + '&quot;);"/>'
                 strHTML += '</div>';
	...
	function chk_buy(strID){
		...
		if(strID == 'null'){
			...
			if(countBuy <= 0){
				location.href="./gameBuy_BuyList.jsp";
			}
			else{
	...
	var chkListOn = function(num){
		
		var chkNum = "chk" + num;
		
		var chkObj = document.getElementById(chkNum);
		if(chkObj.checked){
