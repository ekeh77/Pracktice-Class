	...
	var showBuyList = function(){
		...
		$.ajax({
			...
			success:function(data){
				...
				var buyLists = data["GAMEBUYLISTS"];
				...
				strHTML += '<form action="../GS_BuyGames" method="post" id="buyGame">'
					for(var i=0; i<buyLists.length; i++){
			            ...
			            strHTML += '<span id="deleteSpan" onclick="deleteList(&quot;' + G_ID + '&quot;);">X</span>';
			            strHTML += '</div>';
					}
		         ...
		         strHTML += '<input type="button" value="돌아가기" onclick="location.href=&quot;./gameBuy.jsp&quot;"/>';
		         strHTML += '</div>';
				 strHTML += '</form>'
				 strHTML += '</div>';
		         strHTML += '</div>';
...
var deleteList = function(GID){
	
	$.ajax({
		type: "post",
		async: "true",
		url: "/Test/GS_DeleteBag",
		data:"GID=" + GID,
		dataType:"",
		success:function(data){		
			showBuyList();
...
<body>
	<div id="container">
		...
		<div id="body">
			<div id="menuList">
				<div onclick="location.href='./gameBuy.jsp'">게임</div>
				<div>게임리뷰</div>
				<div>이벤트</div>
				<div>공지사항</div>
				<div>고객지원</div>
