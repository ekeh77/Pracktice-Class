...
INSERT INTO G_GAMES VALUES('G045','Red Dead Redemption 2: Ultimate Edition','GE006',110000);
...
-- 포인트가 부족하면 '포인트가 부족하여 구매할 수 없습니다'라는 문구를 띄워보자
    ...
    BEGIN
        ...
        IF V_M_POINT > V_G_PRICE THEN
            ...
            SET G_POINT = G_POINT - (IN_BUY_PRICE - V_G_PRICE)
            WHERE MID = IN_MID
            ;
...
---------------------------------------------------------------
--포인트로 먼저 구매한후 금액으로 결제
--사용 할 포인트 그후 포인트로 구매금액 빼기
--그 다음 내가 사용할 금액에 구매금액 빼기
--가게 구매내역에 인서트가 필요함
CREATE OR REPLACE PROCEDURE PROC_BUY_GAMES
(
