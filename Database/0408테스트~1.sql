...
INSERT INTO DRIVERS_TBL VALUES('DR003', '홍길동', '010-5555-1254', 'M');
INSERT INTO DRIVERS_TBL VALUES('DR004', '전우지', '010-6666-1254', 'M');
...
--2. 대리기사가 선택한 대리운전 결정 리스트를 보여주세요(5점)
        ...
        SELECT T1.DR_ID, T1.DR_NAME, T3.R_TEL, T3.R_STR, T3.R_DEST, T3.R_PAY, DECODE(T4.F_GUBUN, 1, '완료', 2, '취소')
        FROM DRIVER_RST_TBL T2, DRIVERS_TBL T1, RESERVATION_TBL T3, FINISH_DRIVE_TBL T4
...
--4. 대리운전 기사들의 수익을 가장많은 기사 부터 차례대로 순위를 붙여서 보여 주세요(5점)
        ...
        SELECT * FROM RESERVATION_TBL;
         ...
         SELECT T4.DR_NAME, SUM(T2.R_PAY * 0.8)
         FROM DRIVER_RST_TBL T1, RESERVATION_TBL T2, FINISH_DRIVE_TBL T3, DRIVERS_TBL T4
         WHERE T1.R_ID = T2.R_ID
         AND T1.DR_ID = T4.DR_ID
         AND T1.R_ID = T3.R_ID
         AND F_GUBUN = 1
         GROUP BY T4.DR_NAME
         ORDER BY SUM(T2.R_PAY) DESC
         ;
