...
(
    ...
    INSERT INTO SCORES VALUES('S010', 'P002', 'S002', 1, 1, 89);                             
    INSERT INTO SCORES VALUES('S010', 'P004', 'S004', 1, 1, 78.56);
    INSERT INTO SCORES VALUES('S010', 'P005', 'S005', 1, 1, 80.88);
    INSERT INTO SCORES VALUES('S010', 'P006', 'S006', 1, 1, 98);
    ...
    --2. 교수들중에 평균이 제일 높은 시험을 출제한 교수를 찾아보자
    
    SELECT PID, PNAME, ROUND(AVGSCORE,2) FROM
    (
        SELECT T2.PID,T1.PNAME, AVG(SCORE) AS AVGSCORE, RANK() OVER(ORDER BY AVG(SCORE) DESC) AS RNK
        FROM PROFESSORS T1, SCORES T2
        WHERE T1.PID = T2.PID
        GROUP BY T2.PID, T2.SUBID, T2.EIDX, T1.PNAME
    )
    WHERE RNK = 1
    ;
    
    --3. 부산에 학생들의 총점과 평균 -- 시험친사람
    
    SELECT T2.SID, SUM(SCORE), ROUND(AVG(SCORE),2)
    FROM SCORES T1, STUDENTS T2, REGIONS T3
    WHERE T1.SID = T2.SID
    AND T2.SADDR = T3.RGID
    AND T3.RNAME = '부산'
    GROUP BY T2.SID
    ;
    
    SELECT T1.SID, T1.SID, SUM(SCORE), AVG(SCORE)
    FROM STUDENTS T1, SCORES T2
    WHERE T1.SID = T2.SID
    AND SADDR = 'RG01'
    GROUP BY T1.SID, T1.SID
    ;
    SELECT T1.SADDR, T3.RNAME, SUM(SCORE), AVG(SCORE)
    ...
    SELECT T1.SNAME, T2.SNAME, T3.PNAME, T4.SCORE
    FROM STUDENTS T1, SUBJECTS T2, PROFESSORS T3, SCORES T4
    WHERE T4.SID = T1.SID
    AND T4.SUBID = T2.SUBID
    AND T4.PID = T3.PID
    ;
    
    --시험친 학생들만 --> 출제한 교수별 평균점수 리스트
    
    SELECT T2.PNAME, ROUND(AVG(SCORE),2)
    FROM SCORES T1, PROFESSORS T2
    WHERE T1.PID = T2.PID
    GROUP BY T2.PNAME
    ;
    
    SELECT * FROM PROFESSORS T1,
    (
        SELECT PID, AVG(SCORE) FROM SCORES
        GROUP BY PID
    )T2
    WHERE T1.PID = T2.PID
    ;
    
    --같은지역에 살고 있는 학생과 교수 리스트를 뽑아보자
    --지역명, 학생명, 교수명
    
    SELECT T2.RNAME, T1.SNAME
    FROM STUDENTS T1, REGIONS T2
    WHERE T1.SADDR = T2.RGID
    GROUP BY T2.RGID, T2.RNAME, T1.SNAME
    ORDER BY T2.RNAME
    ;
    
    SELECT T2.RNAME, T1.PNAME
    FROM PROFESSORS T1, REGIONS T2
    WHERE T1.PADDR = T2.RGID
    GROUP BY T2.RGID, T2.RNAME, T1.PNAME
    ORDER BY T2.RNAME
    ;
    
    --여자교수님들이 출제한 시험결과에 제일 좋은 결과를 얻은 학생 - 시험친것만
    
    SELECT T1.SNAME,SCORE FROM STUDENTS T1,
    (
    SELECT T2.SID, T2.SCORE,RANK() OVER(ORDER BY SCORE DESC) AS RANKSCORE
    FROM PROFESSORS T1, SCORES T2
    WHERE T1.PID = T2.PID
    AND PGENDER = 'F'
    ) T2
    ...
    SELECT T1.SID, SUM(T1.SCORE) AS SUMS, ROUND(AVG(SCORE),2) AS AVGS, RANK() OVER(ORDER BY AVG(SCORE) DESC) AS RNK, COUNT(*)
    ...
    SELECT T1.ADATE, T2.SNAME,COUNT(T1.SID)
    FROM ATTENDANCE T1, STUDENTS T2
    WHERE T1.SID = T2.SID
    GROUP BY T1.ADATE, T2.SNAME
    ORDER BY T1.ADATE
    ;
    
    SELECT T1.ADATE, T2.SID, T2.SNAME, COUNT(*)
    FROM ATTENDANCE T1, STUDENTS T2
    WHERE T1.SID = T2.SID
    GROUP BY T1.ADATE, T2.SID, T2.SNAME
    ORDER BY T1.ADATE, T2.SID
    ;
    
    --부산에사는 학생들의 출석률
    
    SELECT A.SID, A.SNAME, COUNT(B.SID) AS CNT, COUNT(*) AS TLTCNT, ROUND(COUNT(B.SID) / COUNT(*) * 100,2) AS PER ,A.RNAME
    FROM
    (
        SELECT TO_CHAR(T1.ADATE, 'YYYY-MM-DD')AS ADATE, T2.SID, T2.SNAME, T3.SUBID, T4.RNAME FROM
            (
                SELECT ADATE FROM ATTENDANCE
                GROUP BY ADATE
            ) T1, STUDENTS T2, SUBJECTS T3, REGIONS T4
            WHERE T2.SADDR = T4.RGID
        ORDER BY T1.ADATE, T2.SID, T3.SUBID
    ) A, ATTENDANCE B
    WHERE A.ADATE = TO_CHAR(B.ADATE(+), 'YYYY-MM-DD')
    AND A.SID = B.SID(+)
    AND A.SUBID = B.SUBID(+)
    AND A.RNAME = '부산'
    GROUP BY A.SID, A.SNAME, A.RNAME
    ;
    
    SELECT T1.ADATE, T2.SNAME,COUNT(T1.SID)
    FROM ATTENDANCE T1, STUDENTS T2, REGIONS T3
    WHERE T1.SID = T2.SID
    AND T2.SADDR = T3.RGID
    AND T3.RNAME = '부산'
    GROUP BY T1.ADATE, T2.SNAME
    ...
    SELECT T4.ADATE,T3.RNAME, ROUND(((1+(COUNT(T1.SID) - COUNT(T2.SID))/COUNT(T2.SID))*100),2)  || '%' AS RATE
    FROM ATTENDANCE T1, STUDENTS T2, REGIONS T3, (SELECT ADATE FROM ATTENDANCE GROUP BY ADATE) T4, SUBJECTS T5
    WHERE T1.SID(+) = T2.SID
    AND T2.SADDR = T3.RGID
    AND T1.ADATE(+) = T4.ADATE
    AND T5.SUBID = T1.SUBID(+)
    AND T3.RNAME = '부산'
    GROUP BY T3.RNAME, T4.ADATE
    ;
    
    --시험안친학생 0점 처리하고 학생, 평균, 총점, 순위
    
    SELECT T3.SNAME, SUM(NVL(T1.SCORE,0)) AS SSUM, ROUND(AVG(NVL(T1.SCORE,0)),2) AS SAVG, RANK() OVER(ORDER BY AVG(NVL(T1.SCORE,0)) DESC) AS RNK
    FROM SCORES T1, EXAMS T2, STUDENTS T3
    WHERE T1.SID(+) = T3.SID
    ...
    --시험을 친학생
    SELECT * FROM SCORES;
    --만약 모든학생이 시험을 다 친 데이터
    SELECT SADDR, SNAME,SUM(TLT), ROUND(AVG(AVG),2), MAX(RNK)
    FROM
    (
        SELECT A.SID, A.SNAME, MAX(C.RNAME) SADDR, SUM(NVL(SCORE, 0)) AS TLT, ROUND(AVG(NVL(SCORE, 0)),2) AS AVG
        ,RANK() OVER(ORDER BY SUM(NVL(SCORE, 0)) DESC) AS RNK
        FROM
        (
            SELECT T1.SID, T1.SNAME, T1.SADDR, T2.PID, T2.SUBID, T2.EIDX
            FROM STUDENTS T1, EXAMS T2
        )A, SCORES B, REGIONS C
        WHERE A.SID = B.SID(+)
        AND A.PID = B.PID(+)
        AND A.SUBID = B.SUBID(+)
        AND A.EIDX = B.EIDX(+)
        ...
        ORDER BY MAX(C.RNAME)
    )
    GROUP BY ROLLUP(SADDR,SNAME)
    ;
    ...
    GROUP BY SADDR
    ;
    SELECT DECODE(PGENDER,'M','남자','F','여자','합계'), CNT
    FROM(
        SELECT PGENDER, COUNT(*) AS CNT
        FROM PROFESSORS
        GROUP BY ROLLUP(PGENDER)
        )
    ;
