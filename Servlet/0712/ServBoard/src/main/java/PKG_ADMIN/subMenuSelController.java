...
@WebServlet("/Admin_SubMenuSel")
public class subMenuSelController extends HttpServlet{
	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		String strSubMenuPid = request.getParameter("subMenuPid");
		...
		try {
			...
			String strProcName = "{call PKG_AD_MENUS.PROC_SUBMENUS_SEL(?,?,?)}";
			CallableStatement csmt = con.prepareCall(strProcName);
			
			csmt.setString(1, strSubMenuPid);
			csmt.setString(2, strSubMenuName);
			csmt.registerOutParameter(3, OracleTypes.CURSOR);
			
			csmt.execute();
			
			ResultSet rs2 = (ResultSet)csmt.getObject(3); // SUBMENULIST
